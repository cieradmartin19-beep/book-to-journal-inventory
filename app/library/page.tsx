...
export default function LibraryPage() {
  ...
  return (
    <AppShell>
      ...
      {!loading && !error && filteredBooks.length === 0 ? <div className="panel mt-5 grid min-h-64 place-items-center p-8 text-center"><div><h2 className="font-serif text-2xl font-black">{books.length ? "No books match these filters" : "Your library is ready"}</h2><p className="mt-2 font-semibold text-ink/65">{books.length ? "Try another search, category, or status." : "Add your first book to begin cataloging."}</p>{!books.length ? <Link href="/add" className="btn-primary mt-5"><Plus size={20} aria-hidden />Add Book</Link> : null}</div></div> : null}
    </AppShell>
  );
}
