...
export default function PublicBookPage({ params }: { params: { shareId: string; bookId: string } }) {
  ...
  if (loading) return <main className="grid min-h-screen place-items-center p-6 font-bold text-ink/65">Loading The Paper Curio...</main>;
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-3 py-4 sm:px-6 sm:py-8">
      <header className="mb-6 flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-lg bg-honey"><BookOpen size={26} aria-hidden /></span><div><p className="font-serif text-2xl font-black">The Paper Curio</p><p className="text-sm font-bold text-ink/60">Curated Books • Handmade Journals • Creative Collections</p></div></header>
      ...
      <section className="grid gap-5 md:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <div className="panel p-5 sm:p-7">
          ...
          <div className="mt-4 flex flex-wrap gap-2"><span className="rounded-md px-2 py-1 text-sm font-black" style={{ backgroundColor: category.color }}>{category.name}</span><span className="rounded-md px-2 py-1 text-sm font-black" style={{ backgroundColor: status.color }}>{status.name}</span></div>
